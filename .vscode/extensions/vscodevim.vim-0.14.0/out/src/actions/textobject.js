 ...
 */
let SelectAnExpandingBlock = class SelectAnExpandingBlock extends TextObjectMovement {
    /**
     * This is a custom action that I (johnfn) added. It selects procedurally
     * larger blocks. e.g. if you had "blah (foo [bar 'ba|z'])" then it would
     * select 'baz' first. If you pressed az again, it'd then select [bar 'baz'],
    ...
    execAction(position, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            const ranges = [
                yield new motion_1.MoveASingleQuotes().execAction(position, vimState),
                yield new motion_1.MoveADoubleQuotes().execAction(position, vimState),
                yield new motion_1.MoveAClosingCurlyBrace().execAction(position, vimState),
                yield new motion_1.MoveAParentheses().execAction(position, vimState),
                yield new motion_1.MoveASquareBracket().execAction(position, vimState),
            ];
