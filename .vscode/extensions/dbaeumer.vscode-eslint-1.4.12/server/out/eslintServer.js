...
messageQueue.registerRequest(vscode_languageserver_1.CodeActionRequest.type, (params) => {
    ...
    for (let editInfo of fixes.getScoped(params.context.diagnostics)) {
        ...
        result.push(vscode_languageserver_1.CodeAction.create(editInfo.label, vscode_languageserver_1.Command.create(editInfo.label, CommandIds.applySingleFix), vscode_languageserver_1.CodeActionKind.QuickFix));
    }
    ;
    if (result.length > 0) {
        let same = [];
        ...
        if (same.length > 1) {
            ...
            result.push(vscode_languageserver_1.CodeAction.create(title, vscode_languageserver_1.Command.create(title, CommandIds.applySameFixes), vscode_languageserver_1.CodeActionKind.QuickFix));
        }
        if (all.length > 1) {
            ...
            result.push(vscode_languageserver_1.CodeAction.create(title, vscode_languageserver_1.Command.create(title, CommandIds.applyAllFixes), vscode_languageserver_1.CodeActionKind.QuickFix));
        }
    }
    return result;
}, (params) => {
