...
function activate(context) {
    ...
    function fixAllProblems() {
        ...
        client.sendRequest(AllFixesRequest.type, { textDocument: { uri } }).then((result) => __awaiter(this, void 0, void 0, function* () {
            if (result) {
                let success = yield applyTextEdits(uri, result.documentVersion, result.edits);
                if (!success) {
                    vscode_1.window.showErrorMessage('TSLint could not apply the fixes');
                }
            }
        }), (_error) => {
